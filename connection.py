...
class Connection:
    ...
    def read_image(self):
        ...
        img_type = ismrmrd.image.get_dtype_from_data_type(image.data_type)
        logging.info("    Image is size %d x %d x %d with %d channels of type %s", image.getHead().matrix_size[0], image.getHead().matrix_size[1], image.getHead().matrix_size[2], image.channels, ismrmrd.image.get_dtype_from_data_type(image.data_type))
        ...
        nbytes = nentries * ismrmrd.image.get_dtype_from_data_type(image.data_type).itemsize

        logging.debug("Reading in %d bytes of image data", nbytes)
        data_bytes = self.read(nbytes)

        image.data.ravel()[:] = np.frombuffer(data_bytes, dtype=ismrmrd.image.get_dtype_from_data_type(image.data_type))
